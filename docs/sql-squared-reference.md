...
SQL² is a subset of ANSI SQL, designed for queries into MongoDB databases.
...
**Note:** Some data types are not natively supported by MongoDB.
Instead, they are emulated by SlamData, meaning that you can use them
as if they were supported by MongoDB.
...
**Note:** If your MongoDB data does not use MongoDB's native date/time type, and instead, you store
...
Unlike a relational database, MongoDB allows data to be nested (that is, data can be
